...
class DDQN(object):
    ...
    # 使DQN系统在环境中运行
    def run_in(self, env, times = 50, need_history = True, need_window = True):
        for episode in range(times):
            ...
            while True:                                                 # 开始一个episode (每一个循环代表一步)
                ...
                print("\r[ episode:{:>4} | steps:{:>6} | reward:{:>6.0f} ]".format(episode + 1, n_steps, episode_reward_sum), end = '')
...
class Environment():
    def __init__(self):
        self.core = gym.make('MountainCar-v0').unwrapped                # 获取环境运行内核
    ...
    def step(self, action):
        state, reward, done, _info = self.core.step(action)             # 从内核中获取信息
        self.n_steps += 1

        x, v = state                                                    # 拆分环境信息

        new_reward = reward + abs(v)*20                                 # 速度越高奖励越高
